...
for i in range(0,12):
   HDF5File["Jillian"].create_dataset(str(i)+"-Jillian", data=pd.read_csv("./Data/"+str(i)+"-Jillian/"+str(i)+"-Jillian.csv", delimiter=',', )
   HDF5File["Jillian"].create_dataset(str(i)+"-Jillian", data=pd.DataFrame(np.genfromtxt("./Data/"+str(i)+"-Jillian/"+str(i)+"-Jillian.csv", delimiter=',')[1:, :]), columns=['Time','X_accel','Y_accel','Z_accel','Abs_accel'])
